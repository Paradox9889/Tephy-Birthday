...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (page2Content) {
    ...
    const animatePage2 = () => {
      texts.forEach(t => {
        t.style.transition = 'opacity 1.3s ease, transform 1.3s ease';
        t.style.opacity = '0';
...
if (page6Content) {
  ...
  // Set up audio-ducking listeners ONCE per audio element (not on every click,
  // to avoid stacking duplicate listeners each time this page is revisited)
  const audioCardMap = { '1': 'song-1', '3': 'memory-3' };
  Object.values(audioCardMap).forEach(audioId => {
    const audioEl = document.getElementById(audioId);
    if (audioEl) {
      audioEl.addEventListener('pause', () => { if (music) music.volume = 0.4; });
      audioEl.addEventListener('play', () => { if (music) music.volume = 0.05; });
      audioEl.addEventListener('ended', () => { if (music) music.volume = 0.4; });
    }
  });
  ...
  const animatePage6 = () => {
    ...
    const memory = document.getElementById('memory-3');
    if (song) { song.pause(); song.currentTime = 0; }
    if (memory) { memory.pause(); memory.currentTime = 0; }
  ...
  cards.forEach((card) => {
    card.addEventListener('click', () => {
      if (!card.classList.contains('flipped')) {
        ...
        // Play the audio card's clip (listeners for ducking are already attached above)
...
if (page7Content) {
  ...
  const animatePage7 = () => {
    ...
    pageFlame.classList.remove('extinguished');
  ...
  if (finalGreeting && letterPaper && heartIcon) {
    ...
    if (restartBtn) {
      restartBtn.addEventListener('click', () => {
        ...
        // Wait for the fade-out to finish before activating Page 1
        // (this delay is what fixes the mobile "can't tap Ready...?" bug)
        setTimeout(() => {
